...
export const metadata: Metadata = {
  ...
  description: 'Финансовая аренда оборудования, техники и транспорта для бизнеса в Узбекистане. Без залога, налоговые льготы.',
};
...
const STEPS = [
  ...
  { n: 3, title: 'Одобрение за 1 день', sub: 'Наш специалист рассмотрит заявку и свяжется с вами для уточнения деталей' },
...
const BENEFITS = [
  ...
  },
  {
    ...
    sub: 'Лизинговые платежи полностью относятся на расходы предприятия и уменьшают налогооблагаемую базу.',
    icon: '📊',
  },
  {
    title: 'Гибкий график',
...
const STATS = [
  ...
  { value: '1 день', label: 'Срок одобрения' },
  { value: 'от 12% (Мурабаха)', label: 'Прибыльная маржа' },
];
...
export default function LeasingPage() {
  return (
    <div>
      ...
      <div style={{ backgroundColor: '#0D5C54' }} className="py-20 px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto">
          <nav className="flex items-center gap-1.5 mb-5 text-xs" style={{ color: 'rgba(232,223,200,0.55)' }}>
            ...
            <span>›</span>
            <span style={{ color: '#E8DFC8' }}>Лизинг</span>
          </nav>
          ...
          <p className="text-lg mb-8" style={{ color: 'rgba(232,223,200,0.75)', maxWidth: 500 }}>
      ...
      {/* Sharia note */}
      <div style={{ backgroundColor: '#094840', borderTop: '1px solid rgba(232,223,200,0.10)' }}>
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-3 text-center">
          <p className="text-xs" style={{ color: 'rgba(232,223,200,0.55)' }}>
            Мы используем структуру Мурабаха — фиксированная наценка вместо процентов
          </p>
        </div>
      </div>
                ...
                <div className="w-14 h-14 rounded-full flex items-center justify-center text-lg font-extrabold mb-4 relative"
                  ...
                  {i < STEPS.length - 1 && (
                    <div className="hidden md:block absolute left-full top-1/2 w-full h-0.5 -translate-y-1/2"
                      style={{ backgroundColor: 'rgba(13,92,84,0.20)', marginLeft: 8 }} />
                  )}
                </div>
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8" style={{ backgroundColor: '#F4EFE4' }}>
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Для бизнеса */}
            <div className="bg-white rounded-2xl p-7" style={{ border: '1px solid #D4C9B0' }}>
                  ...
                  <li key={item} className="flex items-center gap-2.5 text-sm" style={{ color: '#0D1F1D' }}>
                    <div className="w-5 h-5 rounded-full flex items-center justify-center shrink-0"
                      ...
                      <svg width="10" height="10" viewBox="0 0 12 10" fill="none">
                        <path d="M1 5l3.5 3.5L11 1" stroke="#0D5C54" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                      </svg>
                    </div>
                    {item}
                  </li>
      ...
      {/* CTA */}
      <section style={{ backgroundColor: '#094840' }} className="py-20 px-4 text-center">
        <div className="max-w-2xl mx-auto">
          ...
          <p className="text-base mb-8" style={{ color: 'rgba(232,223,200,0.70)' }}>
            Оставьте заявку — наш специалист свяжется с вами в течение 1 рабочего дня и подберёт оптимальные условия
          </p>
          <Link href="/apply?service=leasing"
