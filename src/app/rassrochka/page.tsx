      ...
      <div style={{ backgroundColor: '#0D5C54' }} className="py-20 px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto">
          <nav className="flex items-center gap-1.5 mb-5 text-xs" style={{ color: 'rgba(232,223,200,0.55)' }}>
            ...
            <span>›</span>
            <span style={{ color: '#E8DFC8' }}>Рассрочка</span>
          </nav>
      ...
      <div style={{ backgroundColor: '#094840' }}>
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-5">
          <div className="flex flex-wrap items-center justify-center gap-4">
            <span className="text-sm font-semibold" style={{ color: 'rgba(232,223,200,0.60)' }}>Доступные сроки:</span>
            {TERMS.map(t => (
              <div key={t.months} className="flex items-center gap-1.5 px-4 py-1.5 rounded-full"
                ...
                <span className="text-xs" style={{ color: 'rgba(232,223,200,0.55)' }}>{t.rate}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {CATEGORIES.map(c => (
              <Link key={c.name} href="/catalog"
                ...
                <p className="text-xs font-semibold" style={{ color: '#0D5C54' }}>{c.sub}</p>
